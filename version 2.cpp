	...
	for (int i = 1;i <= N;i++) {
		...
		if (mycity[i].warrior_incity[0] != NULL && mycity[i].warrior_incity[1] != NULL) {
           ...
           //先输出earn的信息，后续再实际earn
			...
			//记录一下战绩,插旗子
			if (mycity[i].warrior_incity[0]->hp > 0 && mycity[i].warrior_incity[1]->hp <= 0)mycity[i].record++;
			else if (mycity[i].warrior_incity[0]->hp <= 0 && mycity[i].warrior_incity[1]->hp > 0)mycity[i].record--;
			else mycity[i].record = 0;//平局就不连续了,只包括battle和arrow，不包括bomb的
